...
test('ajax response with 2 people yields table with 2 rows', function() {
    ...
    visit("/").then(function() {
        var rows = find("table .name").length;
        equal(rows, 2, rows);
    });
});
...
test('add will append another person to the html table', function() {
    ...
    var data = JSON.stringify({firstName: 'dustin', lastName: 'thostenson'});
    stubEndpointForHttpRequest('/api/people', {}, 'POST', 201, data);

    visit("/").then(function() {
      var rows = find("table tr").length
      equal(rows, 3, "the table had " + rows + " rows");
      var fullName = find("table .name:eq(0)").text();
      equal(fullName, "matt morrison", "the first table row had fullName: " + fullName);
      fillIn(".firstName", "dustin");
      fillIn(".lastName", "thostenson");
      return click(".submit");
    }).then(function() {
      equal(find("table tr").length, 4, "the table of people was not complete");
      equal(find("table .name:eq(1)").text(), "dustin thostenson", "dustin was not added to the html table");
  });
});
...
test('delete will remove the person for a given row', function() {
    ...
    visit("/").then(function() {
        var rows = find("table tr").length
        equal(rows, 4, "the table had " + rows + " rows");
        equal(find("table .name:eq(0)").text(), "matt morrison", "the first row was incorrect");
        equal(find("table .name:eq(1)").text(), "toran billups", "the first row was incorrect");
        return click("table .delete:first");
    }).then(function() {
        equal(find("table tr").length, 3, "the table of people was not complete");
        equal(find("table .name:eq(0)").text(), "toran billups", "the wrong person was deleted");
    });
});
